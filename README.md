...
Using **AnyLogic**, the model simulates a full production year (2024) with real historical order data, stochastic machine breakdowns, and variable changeover times.
...
* **Pull-Push Logic:** Orders are prioritized and "pulled" by available machines, then "pushed" to logistics buffers.
* **Custom Agents:**
    *`Order Agent`: Tracks state variables like `procMinutes`, `logisticsCostEur`, and `priorityKey`.
    * `Machine Agent`: Represents resources with distinct attributes for "Old" vs "New" machinery reliability (MTBF/MTTR).
...
* **Failures:** Modeled using exponential distributions based on historical MTBF/MTTR data.
    ...
    * *Legacy Machines:* Frequent breakdowns, slower repair.
* **Changeovers:** Logic-dependent setup times (5-45 mins) based on SKU family sequencing.
***Maintenance:** A **Data-Driven Agent Controller** reads `maintenance_calendar_2024.csv` to trigger scheduled downtime events accurately.
